    ...
    this.http.get<any[]>('http://localhost:3000/api/calendario/alertas')
      .subscribe(data => {
        this.alertas = data.map(alerta => ({
          ...alerta,
          mostrada: false // control local para no repetir alertas
        }));
  ...
  iniciarVerificacion(): void {
    setInterval(() => {
      ...
      for (let alerta of this.alertas) {
        ...
        if (
          Math.abs(ahora.getTime() - fechaAlerta.getTime()) < 15000 && // margen de 15s
          !alerta.mostrada
        ) {
          this.alertaActual = alerta;
          alerta.mostrada = true;

          // Oculta la alerta después de 10 segundos
          setTimeout(() => {
